...
更新时间: 2026-04-30 09:46 CST

## 当前最终状态
- Round 4 已完成、提交并推送到 `main`。
- 最新 commit: `90ee17f75cbe5ca78a8c7335c6051ccf19962829` (`[verified] enforce memory quarantine in recall paths`)。
- `origin/main` 已同步到该 SHA。
- GitHub Actions 对该 SHA 全绿：`docs` success、`test (3.12)` success、install smoke macOS/Ubuntu/Windows success。
- 本地 repo 最后检查 clean：`git status --short --branch` 仅 `## main`。
...
- Review 3 blocked ship：raw `json_extract()` on malformed legacy metadata could raise `OperationalError: malformed JSON`; recall `last_accessed` update also assumed valid JSON. Fixed with guarded SQL and JSONDecodeError fallback.
- Review 4 passed initially but suggested centralizing SQL predicate later; later pre-commit review found SQLite `OR` short-circuit assumption unsafe. Fixed by centralizing `active_memory_sql()` with `CASE WHEN json_valid(...)`.
